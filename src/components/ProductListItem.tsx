...
export default function ProductListItem({ product }: ProductListItemProps) {
  ...
  return (
    <Link href={`/lv/products/${product.id}`} className="md:h-full">
      <div className="flex items-center gap-3 px-4 py-3 hover:bg-brand-pink/10 active:bg-brand-pink/20 transition-colors border-b border-brand-pink/10 last:border-b-0 md:flex-col md:items-stretch md:p-4 md:rounded-xl md:border md:border-brand-pink/30 md:shadow-sm md:hover:shadow-md md:bg-white md:h-full">
        ...
        <div className="md:mt-auto">
          <PriceInquiry productName={displayName} sku={product.sku} />
        </div>
      </div>
    </Link>
  );
